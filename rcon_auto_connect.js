...
exports.rcon_connect = async function(port, i) {
    ...
    client.send = function(cmd) {
        return real_send.call(client, cmd).then(res => {
            ...
            return res;
        }).catch(err => {console.error(`[Rcon]: Error sending command: ${cmd}`, err);});
    };
    ...
    async function connect() {
        ...
        //Reconnect if the attempt failed
            ...
            //Only print the disconnect message once if its an ECONNREFUSED (to prevent spam).
            if(err.code !== 'ECONNREFUSED'){
                console.error(`[Rcon]: Connecting to S${i} failed:`, err.message);
                console.error('Reconnecting in 30 seconds');
            }else if(!HasSendReconnectMsg){
                console.log(`[Rcon]: Could not connect to S${i} (ECONNREFUSED)`);
                console.log('Reconnecting in 30 seconds');
            }
            HasSendReconnectMsg = true;
            Iconnected = false;
            setTimeout(connect, 30e3);
        });
    }
